...
const groups: NavGroup[] = [
  ...
  { label: 'Configuration', items: [{ label: 'People', to: '/people', icon: Users }, { label: 'Extensions', to: '/extensions', icon: Users }, { label: 'Inbound routes', to: '/inbound', icon: Radio }, { label: 'IVR menus', to: '/ivr', icon: Radio }, { label: 'SIP trunks', to: '/trunks', icon: Settings2 }] },
...
function visible(item: NavItem) { return !item.roles || item.roles.includes(props.role ?? 'administrator') }
</script>
...
<template>
  <aside class="rail" :class="{ 'rail--open': open }">
    ...
    <div class="rail__bottom"><button class="profile-chip" @click="emit('signOut')"><span class="profile-chip__avatar">JD</span><span><strong>Jordan Diaz</strong><small>{{ role ?? 'Administrator' }}</small></span><LogOut :size="15" class="muted" /></button></div>
  </aside>
</template>
